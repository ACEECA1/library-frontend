...
export function ModerationQueue() {
  ...
  const handleApprove = async (id: number) => {
    try {
      await api.post(`/books/${id}/approve`);
            ...
            <div key={book.id} className="flex flex-col sm:flex-row items-center justify-between p-4 border rounded-lg bg-gray-50">
              <div className="flex items-center gap-4 w-full sm:w-auto mb-4 sm:mb-0">
                <div className="w-12 h-16 bg-gray-200 flex items-center justify-center rounded">
                  {book.thumbnailUrl ? (
                    <img src={book.thumbnailUrl} alt="cover" className="w-full h-full object-cover rounded" />
                  ) : (
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                {/* Note: View details button could open a modal or new page */}
                <button 
