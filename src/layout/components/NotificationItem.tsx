...
export function NotificationItem({ notification, onMarkRead }: { notification: any, onMarkRead: (id: number) => void }) {
  return (
    <div className={`px-4 py-3 border-b border-gray-50 flex items-start gap-3 hover:bg-gray-50 transition-colors ${notification.isRead ? 'opacity-60' : 'bg-green-50/30'}`}>
      <div className={`w-2 h-2 mt-1.5 rounded-full shrink-0 ${notification.isRead ? 'bg-transparent' : 'bg-[#00502D]'}`}></div>
