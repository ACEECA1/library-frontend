            ...
            <div className="relative group cursor-pointer">
              ...
              <div className="absolute right-0 top-full pt-2 w-48 hidden group-hover:block z-50">
                <div className="bg-white rounded-lg shadow-xl py-2 text-gray-800 border border-gray-100">
                  <Link to="/collection" className="flex items-center gap-2 px-4 py-2 hover:bg-gray-100"><BookMarked size={16} /> {t('nav.bookmarks', 'My Collection')}</Link>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        >
          <nav className="p-4 flex flex-col gap-2 w-64 flex-1">
            <SidebarLink to="/" icon={<Home size={20} />} label={t('nav.home', 'Home')} active={location.pathname === '/'} />
            <SidebarLink to="/browse" icon={<Library size={20} />} label={t('nav.library', 'Browse Catalog')} active={location.pathname.startsWith('/browse')} />
            <SidebarLink to="/collection" icon={<BookMarked size={20} />} label={t('nav.bookmarks', 'My Collection')} active={location.pathname === '/collection'} />
            {hasAdminDashboardAccess && (
              <>
